/***********************************************************************
Pong Clock v 5.2, Oct 2013 by Nick Hall
...
#define NUM_SETTINGS_MODES 7             // Number settings modes = 8 (conting zero as the first mode)
#define NUM_DISPLAY_MODES 5              // Number display modes = 6 (conting zero as the first mode)
...
int dark_mode_active = 0;               // Track if switching LEDOFF / LEDON has occurred so the routine isn't run over and over again
...
void set_drkmode() {
    screen_on();
    // dark mode - flip the bit (makes 0 into 1, or 1 into 0 for dark mode)
    if (dark_mode == 0){
        dark_mode = 1;
    } else {
        dark_mode = 0;
    ...
    if (rtc[2] > clock_off - 1 || rtc[2] < clock_on - 1) {
        dark_mode_active = 0;
    } else
    {
        dark_mode_active = 1;
...
//set dark mode on or off for the weekend
void set_weekend() {
    screen_on();
    // dark mode - flip the bit (makes 0 into 1, or 1 into 0 for dark mode)
    if (weekend_mode == 0){
    ...
    if (rtc[3] == 0 || rtc[3] == 6) {
        weekend_mode_active = 0;
    } else
    {
        weekend_mode_active = 1;
    }
...
void night_mode()
{
    if ((rtc[2] > clock_off - 1 || rtc[2] < clock_on - 1) && dark_mode_active == 0) {
        screen_off();
        dark_mode_active = 1;
    }
    if ((rtc[2] > clock_on - 1 && rtc[2] < clock_off - 1) && dark_mode_active == 1) {
        screen_on();
...
void weekend_night_mode()
{
    if ((rtc[3] == 0 || rtc[3] == 6) && weekend_mode_active == 0) {
        screen_off();
        weekend_mode_active = 1;
    }
    if ((rtc[3] > 0 && rtc[3] < 6) && weekend_mode_active == 1) {
        screen_on();
        if (clock_mode == 0 || clock_mode == 4) {
            ht1632_putchar( 12, 0, ':');
            ht1632_putchar( 30, 0, ':');
 ...
 */
void screen_off() {
    ht1632_sendcmd(0, HT1632_CMD_LEDOFF); //send intensity commands using CS0 for display 0
    ht1632_sendcmd(1, HT1632_CMD_LEDOFF); //send intensity commands using CS0 for display 1
    cls();
}
 ...
 */
void screen_on() {
    ht1632_sendcmd(0, HT1632_CMD_LEDON); //send intensity commands using CS0 for display 0
    ht1632_sendcmd(1, HT1632_CMD_LEDON); //send intensity commands using CS0 for display 1
}
...
void get_time()
{
    ...
    if((dark_mode == 1 && weekend_mode == 0) || (rtc[3] > 0 && rtc[3] < 6 && weekend_mode == 1 && dark_mode == 1)) {
        night_mode();  //Check to see if the digits should fade to black at night or back on during day
    }

    if(weekend_mode == 1) {
        weekend_night_mode();  //Check to see if the digits should fade to black for the weekend
    }
